...
def download() -> None:
    data_url = "https://huggingface.co/datasets/OmAlve/TinyStories-Hindi/resolve/main/TinyStories_hindi_all_data.tar.gz"
    ...
    if not data_dir.exists():
        ...
        logging.info("Extracting TinyStories dataset...")
    ...
    with open(tiny_file, "w") as f:
        for shard in shard_filenames[:10]:
            with open(shard, "r") as g:
                data = json.load(g)
            for example in data:
                f.write(example["translated"].strip() + "\n")
